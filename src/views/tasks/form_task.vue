<template>
  <form-modal @close-menu="$emit('close-menu')" :loading="loading">
    <template v-slot:title>
      <div v-if="fields.id == null">Создать новую задачу</div>
      <div v-else>Обновить задачу</div>
    </template>
    <template v-slot:body>
      <div>
        <div v-for="(el, key) in $store.getters['db/structure'].crm_tasks" :key="key">
          <edit-element
            :el="el"
...
export default {
  ...
  props: {
    data: {
      type: Object,
      default: function() {
        return this.$store.getters["db/fields"]("crm_tasks");
      }
    }
  },
  data: function() {

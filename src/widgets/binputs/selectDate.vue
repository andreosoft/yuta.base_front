<template>
  <div class="form-control" style="position: relative; padding: 3px 0 0 0;">
    <div style="position: relative;">
      <input
        tabindex="-1"
        :class="class_input"
        class="form-control vdp-select-placeholder"
        :value="res_placeholder"
      />
      <input
        class="vdp-select-input"
        :disabled="disabled"
        :value="value"
        :name="name"
...
export default {
  props: {
    ...
    setTimeCaption: { type: String, default: "Установите время:" },
  ...
  watch: {
    value: function(value, previousValue) {
      ...
      if (
        ...
        val = this.formatMask(value, this.template);
      } else {
        val = value;
      }
      ...
      this.$emit("input", val);
      this.currentPeriod = this.getPeriodFromValue(val, this.format);
    }
  },
  created() {
  ...
  methods: {
    formatMask: function(text, wholeMask) {
      if (!wholeMask) return text;

      var maskStartRegExp = /^([^0ANX]+)/;
    ...
    selectDateItem(item) {
      if (!item.disabled) {
        ...
        let d = this.formatDateToString(newDate, this.format);
        this.res_placeholder = d;
        this.$emit("input", d);
